                ...
                <div class="col-lg-12">
                    ...
                    <div class="mb-10">
                        ...
                        <div class="row mx-0 mb-3">
                            <div class="col-md-3 bg-light text-center py-5 rounded">
                                <div class="fw-bold fs-6 mb-3">Lampiran saat ini</div>
                                <i class="bi bi-file-earmark fs-3x"></i>
                                <div class="mt-3">
                                    <div class="mb-3">
                                        <?php
                                        $explodedAttachment = explode('/', $announcement->attachment);
                                        $fileName = $explodedAttachment[count($explodedAttachment) - 1] ?? "";
                                        ?>
                                        <span class="text-muted" target="_blank">{{ $fileName }}</span>
                                    </div>
                                    <div>
                                        <a href="{{ $announcement->attachment }}" target="_blank" class="btn btn-sm btn-primary w-100"> <i class="bi bi-box-arrow-up-right"></i><span>Buka</span></a>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    const app = new Vue({
        ...
        data() {
            return {
                model: {
                    isAllCompanies: Number('{{ $announcement->is_all_companies }}'),
                    companyIds: JSON.parse('{{ $announcement->company_ids }}'),
        ...
        methods: {
            ...
            async onSubmit() {
                ...
                try {
                    ...
                    const response = await axios.post('/announcements/{{ $announcement->id }}', formData);
    ...
    $(function() {
        $('#input-date-end-date').daterangepicker({
            ...
            startDate: moment('{{ $announcement->end_date }}'),
            // minYear: 1901,
            // maxYear: parseInt(moment().format('YYYY'), 10)
        }, function(start, end, label) {
