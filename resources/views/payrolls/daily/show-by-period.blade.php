    ...
    const app = new Vue({
        ...
        methods: {
            ...
            openBulkDeleteConfirmation() {
                ...
                Swal.fire({
                    title: 'Apakah anda yakin?',
                    text: (self?.model?.checkedSalariesIds.length || '') + " Data akan dihapus",
                    icon: 'warning',
                    reverseButtons: true,
                    showCancelButton: true,
                    confirmButtonText: 'Hapus',
            ...
            sendBulkDeleteRequest() {
                const self = this;
                return axios.delete('/daily-salaries/action/bulk-delete' + '?ids=' + JSON.stringify(self?.model?.checkedSalariesIds.map(id => Number(id))))
                    .then(function(response) {
                        let message = response?.data?.message;
                        if (!message) {
                            message = 'Data berhasil disimpan'
