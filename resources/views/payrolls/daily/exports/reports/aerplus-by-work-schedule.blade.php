...
<table>
    <tbody>
        <tr>
            <td colspan="{{ $columnsCount }}" style="text-align: center;"><strong>Rekapitulasi Gaji Harian AerPlus (By Work Schedule)</strong></td>
        </tr>
        <tr>
        ...
        $allowance = $employee['allowance'] ?? collect($additionalIncomes)->filter(function ($item) {
            return str_starts_with($item['type'] ?? '', 'tunjangan');
        })->sum('value');
        $redeemDeposit = $employee['redeem_deposit'] ?? collect($additionalIncomes)->where('type', 'redeem_deposit')->sum('value');
        $lateFee = $employee['late_fee'] ?? collect($deductions)->where('type', 'late_fee')->sum('value');
        $deposit = $employee['deposit'] ?? collect($deductions)->where('type', 'deposit')->sum('value');
        $loan = $employee['loan'] ?? collect($deductions)->where('type', 'loan')->sum('value');
