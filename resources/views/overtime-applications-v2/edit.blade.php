  ...
  const app = new Vue({
    ...
    methods: {
      ...
      async save() {
        let self = this;
        try {
          if (this.model.type != "other" && (this.model.orderQuantity == "" || this.model.orderQuantity == 0)) {
            Swal.fire({
              title: 'Kesalahan',
          ...
          const response = await axios.post('/overtime-applications-v2/{{ $overtime_application->id }}', {
            number: model.number,
            date: model.date,
            type: model.type,
