        ...
        <div class="col-lg-4">
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="m-0 text-gray-700 fw-bold fs-4">{{ $current_private_insurance->name }}</p>
                </div>
            </div>
        </div>
        <div class="col-lg-4">
            <div class="card">
                <div class="card-body">
                    <h3 class="m-0 mb-5 text-info">Premi Tabungan</h3>
                    <p class="m-0 text-gray-700 fw-bold fs-4">Rp. {{ number_format($current_private_insurance->premi_tabungan, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
        <div class="col-lg-4">
            <div class="card">
                <div class="card-body">
                    <h3 class="m-0 mb-5 text-info">Premi Kematian</h3>
                    <p class="m-0 text-gray-700 fw-bold fs-4">Rp. {{ number_format($current_private_insurance->premi_kematian, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
    </div>
                            ...
                            <div class="mb-10">
                                ...
                                <select class="form-select form-select-solid" id="filterInsuranceYear">
                                    ...
                                    for ($i = 2020; $i <= $limitYear; $i++) {
                                        $nYear = $i;
                                    ?>
                                        <option value="<?= $nYear ?>" <?= request()->query('year') == $nYear ? 'selected' : '' ?>><?= $nYear ?></option>
                                    <?php
                                    }
                                    ?>
                                </select>
                ...
                <table class="table align-middle table-row-dashed fs-6 gy-5">
                    ...
                    <thead>
                        ...
                        <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0 align-middle">
                            ...
                            <th class="min-w-125px" rowspan="2">No</th>
                            ...
                            <th colspan="2" class="text-center {{ $nextYearValueClass }}">Pembayaran Polis Tahun {{ $current_year }}</th>
                            <th class="text-center {{ $nextYearValueClass }} min-w-200px" rowspan="2">Akumulasi Nilai Tabungan Tahun {{ $current_year }}</th>
                        </tr>
                        <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                            <!-- 8 -->
                            <th class="min-w-125px text-center {{ $nextYearValueClass }}">Total Premi</th>
                            <th class="min-w-125px text-center {{ $nextYearValueClass }}">Kesehatan</th>
                        </tr>
                        <!--end::Table row-->
                    </thead>
                    <!--end::Table head-->
                    ...
                    <tbody class="fw-bold text-gray-600">
                        <tr v-for="(employee, index) in employees">
                            ...
                            <td class="text-center">@{{ employee?.current_private_insurance?.pivot?.start_year || '' }}</td>
                            <td class="text-end {{ $nextYearValueClass }}">
                                <input type="number" v-model="employee.current_year_value.total_premi" class="form-control form-control-sm w-100">
                            </td>
                            <td class="text-end {{ $nextYearValueClass }}">
                                <input type="number" v-model="employee.current_year_value.kesehatan" class="form-control form-control-sm w-100">
                            </td>
                            <td class="text-end {{ $nextYearValueClass }}">
                                <input type="number" v-model="employee.current_year_value.nilai_tabungan" class="form-control form-control-sm w-100">
                            </td>
                        </tr>
                    </tbody>
                    <!--end::Table body-->
                    <tfoot>
                        <tr class="bg-light-primary">
                            ...
                            <td class="text-end">@{{ currencyFormat(insuranceTotal.totalPremi) }}</td>
                            <td class="text-end">@{{ currencyFormat(insuranceTotal.kesehatan) }}</td>
                            <td class="text-end">@{{ currencyFormat(insuranceTotal.nilaiTabungan) }}</td>
                        </tr>
                    </tfoot>
                </table>
                <!--end::Table-->
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                privateInsuranceId: '{{ $current_private_insurance->id }}',
                loading: false,
            }
        },
        computed: {
            insuranceTotal() {
                // PREVIOUS
                const totalTotalPremi = this.employees.map(employee => Number(employee.current_year_value.total_premi)).reduce((acc, cur) => acc + cur, 0);
                const totalKesehatan = this.employees.map(employee => Number(employee.current_year_value.kesehatan)).reduce((acc, cur) => acc + cur, 0);
                const totalNilaiTabungan = this.employees.map(employee => Number(employee.current_year_value.nilai_tabungan)).reduce((acc, cur) => acc + cur, 0);

                return {
                    totalPremi: totalTotalPremi,
                    kesehatan: totalKesehatan,
                    nilaiTabungan: totalNilaiTabungan,
                }
            }
        },
        methods: {
            async onSaveBpjsValues() {
                ...
                try {
                    ...
                    const response = await axios.post('/private-insurance-values', {
                        employees,
                        previous_year: previousYear,
                        current_year: currentYear,
                        private_insurance_id: privateInsuranceId,
                    });
