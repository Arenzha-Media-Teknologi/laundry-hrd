...
@section('title', 'Laporan ' . $private_insurance->name)
        ...
        <div class="card card-flush py-4">
            ...
            <div class="card-header">
                <div class="card-title">
                    <h2>Rekapitulasi {{ $private_insurance->name }}</h2>
                </div>
                    ...
                    <div class="division-table">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 text-end">
                                <a href="/reports/insurances/privates/{{ $private_insurance->id ?? '' }}/export?company={{ $filter['company_id'] ?? '' }}&division={{ $filter['division_id'] ?? '' }}&year={{ $filter['year'] ?? '' }}" class="btn btn-success" target="_blank"><i class="bi bi-download"></i> Download .xlsx</a>
                            </div>
                        </div>
                            ...
                            <table class="table table-rounded border gy-7 gs-7">
                                <thead>
                                    <tr class="fw-bold fs-6 text-gray-800 border-bottom border-gray-200">
                                        ...
                                        <th rowspan="2" class="min-w-200px">Nama Pemegang Polis</th>
                                        <th rowspan="2" class="min-w-200px">Polis</th>
                                        <th rowspan="2" class="min-w-200px">Tahun Mulai Polis</th>
                                        <th colspan="3" class="min-w-200px text-center">Pembayaran Premi Polis Tahun 2022</th>
                                        <th rowspan="2" class="min-w-200px text-end">Akum. Nilai Tabungan Tahun 2022</th>
                                        <th rowspan="2" class="min-w-200px text-end">Polis Kematian</th>
                                    </tr>
                                    <tr class="fw-bold fs-6 text-gray-800 border-bottom border-gray-200">
                                        <th class="min-w-200px text-end">Total Premi</th>
                                        <th class="min-w-200px text-end">Kesehatan</th>
                                        <th class="min-w-200px text-end">Tabungan</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end">{{ number_format($employee->current_year_value[$property] ?? 0, 0, ',', '.') }}</td>
                                        @endforeach
                                    </tr>
                                    @endforeach
                                    <tr class="fw-bold fs-6 text-gray-800 border-bottom border-gray-200">
                                        ...
                                        ?>
                                        @foreach($properties as $property)
                                        <td class="text-end">{{ number_format($division->subtotal[$property] ?? 0, 0, ',', '.') }}</td>
                                        @endforeach
                                    </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
                                    <tr class="fw-bold fs-6 text-gray-800 border-bottom border-gray-200">
                                        ...
                                        ?>
                                        @foreach($properties as $property)
                                        <td class="text-end">{{ number_format($grand_totals[$property] ?? 0, 0, ',', '.') }}</td>
                                        @endforeach
                                    </tr>
                                </tfoot>
                            </table>
...
<script>
    ...
    const privateInsurance = <?php echo Illuminate\Support\Js::from($private_insurance) ?>;
    ...
    const app = new Vue({
        ...
        methods: {
            applyFilter() {
                ...
                document.location.href = `/reports/insurances/privates/${privateInsurance?.id}?company=${companyId}&division=${divisionId}&year=${year}`;
            }
        }
    })
</script>
