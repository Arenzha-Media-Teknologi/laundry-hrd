                    ...
                    <div class="division-table">
                        <div class="row">
                            ...
                            <div class="col-md-6 text-end">
                                <a href="/reports/insurances/insurances-list-export?company={{ $filter['company_id'] ?? '' }}&division={{ $filter['division_id'] ?? '' }}&year={{ $filter['year'] ?? '' }}" class="btn btn-success" target="_blank"><i class="bi bi-download"></i> Download .xlsx</a>
                            </div>
                        </div>
                            ...
                            <table class="table table-rounded border gy-7 gs-7">
                                <thead>
                                    <tr class="fw-bold fs-6 text-gray-800 border-bottom border-gray-200">
                                        ...
                                        <th class="min-w-200px">Tahun Mulai Polis</th>
                                        <th class="min-w-200px">Jenis Polis</th>
                                        <th class="min-w-200px text-end">Nominal</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($divisions as $division)
                                    <tr class="bg-light-primary">
                                        <td colspan="7" class="fw-bold fs-6 text-gray-800">{{ $division->name }} - {{ $division->company->name ?? '' }}</td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end fw-bold">{{ number_format(($employee->insurance_values[0]['value'] ?? 0), 0, ',', '.') }}</td>
                                        @else
                                        <td></td>
                                        <td></td>
                                        <td></td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end fw-bold">{{ number_format(($insuranceValue['value'] ?? 0), 0, ',', '.')  }}</td>
                                    </tr>
                                    @endif
                                    @endforeach
                                    @endif
                                    ...
                                    <tr class="fw-bold fs-6 text-gray-800 border-bottom border-gray-200">
                                        ...
                                        <td class="text-end fw-bold fw-5">{{ number_format(($division->subtotal ?? 0), 0, ',', '.') }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
                                    <tr class="fw-bold fs-6 text-white border-bottom border-gray-200 bg-success">
                                        <td colspan="6">Grand-Total</td>
                                        <td class="text-end fw-bold fs-3">Rp {{ number_format(($grand_total ?? 0), 0, ',', '.') }}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
    ...
    const app = new Vue({
        ...
        methods: {
            applyFilter() {
                ...
                document.location.href = `/reports/insurances/insurances-list?company=${companyId}&division=${divisionId}&year=${year}`;
            }
        }
    })
</script>
